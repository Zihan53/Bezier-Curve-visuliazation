...
decasteljauInput.addEventListener("focusout", function(){
    ...
    curve.setDecasteljauStep(input_value);
    draw(curve);
});

var stepInc = document.getElementById("stepIncrease");
stepInc.addEventListener("click", function(){
    var input_value = parseInt(decasteljauInput.value);
    if (input_value < curve.getDecasteljauMax()) {
        input_value += 1;
...
var stepDec = document.getElementById("stepDecrease");
stepDec.addEventListener("click", function(){
    var input_value = parseInt(decasteljauInput.value);
    if (input_value > 0) {
        input_value -= 1;
        decasteljauInput.value = input_value;
...
tInput.addEventListener("focusout", function(){
    var input_value = tInput.value;
    if (input_value > 1) {
        input_value = 1;
    } else if (input_value < 0) {
        input_value = 0;
    }
    tInput.value = input_value;
    curve.setT(input_value);
    draw(curve);
});
